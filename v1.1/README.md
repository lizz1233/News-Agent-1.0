...
The agent runs daily at 8:00 AM via a launchd job. The plist lives at `~/Library/LaunchAgents/com.news-agent.daily.plist` and sources `.zshrc` for the API key.
...
To change the time, edit the plist — change `Hour` (0–23) and `Minute` — then unload and reload.
